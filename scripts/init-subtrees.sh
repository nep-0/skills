...
if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "Working tree has uncommitted changes. Commit or stash them before initializing." >&2
  exit 1
fi
...
for index in $(seq 0 "$((count - 1))"); do
  ...
  if [[ -e "$local_prefix" ]]; then
    echo "Skipping $name: $local_prefix already exists."
    skipped="$((skipped + 1))"
    continue
  fi

  safe_name="$(printf '%s' "$name" | tr -c '[:alnum:]._-' '-')"
  split_branch="init-subtree-${safe_name}"
  ...
  echo "Initialized $local_prefix from $remote/$branch:$upstream_prefix"
  initialized="$((initialized + 1))"
done

if [[ -n "$ONLY" && "$initialized" -eq 0 && "$skipped" -eq 0 ]]; then
  echo "No subtree named '$ONLY' found in $CONFIG." >&2
  exit 1
fi

echo "Initialized $initialized subtree(s), skipped $skipped existing subtree(s)."
